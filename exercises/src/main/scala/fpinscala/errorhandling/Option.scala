...
object Option {
  ...
  def sequence[A](a: List[Option[A]]): Option[List[A]] =
    a.foldRight[Option[List[A]]](Some(Nil))((n, p) => map2(n, p)(_ :: _))

  def traverse[A, B](a: List[A])(f: A => Option[B]): Option[List[B]] =
    a.foldRight[Option[List[B]]](Some(Nil))((c, acc) => map2(f(c), acc)(_ :: _))

  def main(args: Array[String]): Unit = {
    println(Some(1).orElse(None))
    println(None.orElse(Some(1)))
    println(sequence(List(Some(1), Some(2), Some(3))))
    println(sequence(List(Some(1), Some(2), None)))

    println(traverse(List(1, 2, 3))(i => Some(i.toDouble)))
  }
}